        ...
        if (targetPlayer == null) {
            ...
            if (playerFaction == null) {
                ...
                if (!playerFaction.getPlayersIn().contains(targetPlayer.getUniqueId())) {
                    player.sendMessage("§cError: This player is not in your faction !");
                } else {
                    if (!playerFaction.getOwner().equals(player.getUniqueId())) {
                        player.sendMessage("§cError: You're not the owner of your faction !");
                    } else {
                        factionSQL.getPlayersInFactionSQL().makePlayerLeave(playerFaction, targetPlayer.getUniqueId());
                        playerFaction.getPlayersIn()
                                .stream()
                                .map(Bukkit::getPlayer)
                                .filter(Objects::nonNull)
                                .forEach(player1 -> player1.sendMessage("§b" + targetPlayer.getName() + "§c has been kicked from your faction !"));
                        if (targetPlayer.isOnline()) {
                            ((Player) targetPlayer).sendMessage("§cYou have been kicked from the faction §b" + playerFaction.getName() + "§a !");
                        }
                    }
                }
            }
